    ...
    <>
      <Box sx={{ width: '100%' }}>
        ...
        <TabPanel value={value} index={0}>
          <FollowersList followers={followingsList} onUpdateFollowings={updateFollowings} myProfile={props.myProfile} />
        </TabPanel>
        <TabPanel value={value} index={1}>
          <FollowersList followers={followersList} onUpdateFollowings={updateFollowings} myProfile={props.myProfile} />
        </TabPanel>
      </Box>
    </>
  );
              ...
              <React.Fragment>
                <Link to={'/' + user.user.username}>
                  <Stack direction="row" spacing={2}>
                    ...
                    {props.myProfile?.username === user.user.username ? null : props.followers.find((followingUser) => {
                        console.log({ followingUser });
                        return followingUser.followedUserId === props.myProfile?.id;
                      }) ? (
                      <Button
                        size="small"
                        onClick={(e) => {
                          props.onUpdateFollowings(user.user.username, 'unFollow');
                          e.preventDefault();
                        }}
                      >
                        フォロー解除
